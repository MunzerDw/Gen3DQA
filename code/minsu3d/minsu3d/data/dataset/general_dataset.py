...
from minsu3d.util.transform import elastic, flip, jitter, rotz
...
class GeneralDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg.model.model.use_height:
            floor_height = np.percentile(points[:,2],0.99)
            heights = np.expand_dims(points[:, 2] - floor_height, 1) # (N, 1)
            feats = np.concatenate((feats, heights), axis=1)
